...
void In::calcFC2()
{
	...
	if (order ==2){
        ...
        int *nepp;
        mem->allocate(nepp, geo->nprocs); // number elements (FCs) per proc
        for (int p=0; p<geo->nprocs; p++){
            nepp[p] = nfc2/geo->nprocs;
        }

        // divide up the remainder
        for (int p=0; p<(nfc2 % geo->nprocs); p++){
            nepp[p] += 1;
        }
	...
	if (order ==3){
        ...
        mem->allocate(nepp, geo->nprocs); // number elements (FCs) per proc
        for (int p=0; p<geo->nprocs; p++){
            nepp[p] = nfc3/geo->nprocs;
        }

        // divide up the remainder
        for (int p=0; p<(nfc3 % geo->nprocs); p++){
            nepp[p] += 1;
        }
	...
	if (order ==4){
        ...
        */
        ...
        mem->allocate(nepp, geo->nprocs); // number elements (FCs) per proc
        for (int p=0; p<geo->nprocs; p++){
            nepp[p] = nfc4/geo->nprocs;
        }

        // divide up the remainder
        for (int p=0; p<(nfc4 % geo->nprocs); p++){
            nepp[p] += 1;
        }
