...
Verify::Verify(GEO *geo) : Ptrs(geo) {
    ...
    for (int m=0; m<nconfigs; m++){
        ...
        for (int n=0; n<nmodes; n++){
            ...
            for (int i=0; i<lmp->atom->natoms; i++){
                ...
                q[n] += sqrt(mass_convert)*geo->in->emat[0+(i*3)][n]*u[i][0]*1e-10;
                q[n] += sqrt(mass_convert)*geo->in->emat[1+(i*3)][n]*u[i][1]*1e-10;
                q[n] += sqrt(mass_convert)*geo->in->emat[2+(i*3)][n]*u[i][2]*1e-10;
            }
            //printf(" q[%d]: %e\n", n,q[n]);
        }
        ...
        for (int i=0; i<lmp->atom->natoms; i++){
            ...
            u[i][2] = 0.0;
            for (int n=0; n<nmodes; n++){
                u[i][0] += (1.0/sqrt(mass_convert))*geo->in->emat[0+(i*3)][n]*q[n];
                u[i][1] += (1.0/sqrt(mass_convert))*geo->in->emat[1+(i*3)][n]*q[n];
                u[i][2] += (1.0/sqrt(mass_convert))*geo->in->emat[2+(i*3)][n]*q[n];
                //printf(" mass[type[%d]] = %f\n", n,mass[type[n]]);
            }
...
double Verify::calcCohesiveEnergy()
{
    ...
    for (int i=0;i<lmp->atom->natoms;i++){
        //printf(" %f %f %f\n", u[i][0],u[i][1],u[i][2]); 
        x[0+i*3]=geo->in->x0[i][0];
        x[1+i*3]=geo->in->x0[i][1];
        x[2+i*3]=geo->in->x0[i][2];
    }
...
double Verify::calcEnergyLmp(double **u)
{
    ...
    for (int i=0;i<lmp->atom->natoms;i++){
        //printf(" %f %f %f\n", u[i][0],u[i][1],u[i][2]); 
        x[0+i*3]=geo->in->x0[i][0] + u[i][0];
        x[1+i*3]=geo->in->x0[i][1] + u[i][1];
        x[2+i*3]=geo->in->x0[i][2] + u[i][2];
    }
...
double Verify::calcEnergyMtep(double *q)
{
    ...
    for (int i=3; i<nmodes; i++){
        ...
        for (int j=3; j<nmodes; j++){
            if (i==j){
                printf(" q[%d]: %e\n", i,q[i]);
                e += 0.5*4*pi*pi*geo->in->freqs[i]*geo->in->freqs[i]*q[i]*q[i]*6.242e+18; // Need to convert harmonic part to eV
            }
