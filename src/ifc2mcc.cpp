...
Ifc2mcc::Ifc2mcc(GEO *geo) : Ptrs(geo) {
...
void Ifc2mcc::go()
{
    ...
    */
    ...
    if (order==3){
        ...
        int *nepp;
        mem->allocate(nepp, geo->nprocs); // number elements (MCC3s) per proc
        for (int p=0; p<geo->nprocs; p++){
            nepp[p] = n/geo->nprocs;
        }

        // divide up the remainder
        for (int p=0; p<(n % geo->nprocs); p++){
            nepp[p] += 1;
        }
...
void Ifc2mcc::extract()
{
    ...
    int nfiles = 3; // Number of files to scan through, ID starting at zero.
